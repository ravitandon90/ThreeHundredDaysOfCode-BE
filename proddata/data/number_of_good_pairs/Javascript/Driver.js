...
const numIdenticalPairsFn = new Solution().numIdenticalPairs;
...
function main() {
  for (let i = 0; i < sizeOfTestCases; i++) {
    const user_code_output = numIdenticalPairsFn(testcases[i].arg1);
...
function getTestCases() {
  try {
    let data = fs.readFileSync(
      "proddata/data/number_of_good_pairs/testcases.txt",
      "utf8"
    );
    data = data.split("\n");
    data.shift();
