...
func main() {
	...
	for lineNumber, line := range lines[1:] {
		...
		if lineNumber%2 == 0 {
			...
			s1 := strings.TrimRight(input[0], " ")
			s1 = strings.TrimSpace(s1[1 : len(s1)-1])
			s2 := strings.TrimLeft(input[1], " ")
			s2 = strings.TrimSpace(s2[1 : len(s2)-1])
			actualOutput = uncommonFromSentences(s1, s2)
		} else {
