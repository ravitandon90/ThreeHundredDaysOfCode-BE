...
function main() {
  for (let i = 0; i < sizeOfTestCases; i++) {
    let input_arr = testcases[i].arg1; // input_arr is passed as a reference and changes are being made on the same array
    Solution(input_arr, testcases[i].arg2);

    if (!compare1DArray(input_arr, testcases[i].output)) {
      console.log("Result: Failed");
      console.log("Failed test case:", testcases[i].arg1, testcases[i].arg2);
      console.log("Output:", input_arr);
...
function getTestCases() {
  try {
    let data = fs.readFileSync(
      "proddata/data/rotate_array/testcases.txt",
      "utf8"
    );
    data = data.split("\n");
