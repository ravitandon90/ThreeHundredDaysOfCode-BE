...
func main() {
	...
	for lineNumber, line := range lines[1:] {
		...
		if lineNumber%2 == 0 {
			...
			times := createIntMatrixFromString(line[:ind+1])
			for line[ind] != ',' {
				ind++
			}
			otherInputs := strings.Split(line[ind+1:], ",")
			n, _ := strconv.Atoi(strings.TrimSpace(otherInputs[0]))
			k, _ := strconv.Atoi(strings.TrimSpace(otherInputs[1]))
			actualOutput = networkDelayTime(times, n, k)
		} else {
