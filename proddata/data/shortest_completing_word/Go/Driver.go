...
func main() {
	...
	for lineNumber, line := range lines[1:] {
		...
		if lineNumber%2 == 0 {
			...
			licensePlate := strings.TrimRight(line[:ind], " ")
			licensePlate = strings.TrimSpace(licensePlate[1 : len(licensePlate)-1])
			words := createStringArray(strings.TrimLeft(line[ind+1:], " "))
			actualOutput = shortestCompletingWord(licensePlate, words)
		} else {
